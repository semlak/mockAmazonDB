...
module.exports = class ORM {
	...
	static insertIfNotPresent(item, originalCallback) {
		...
		let callback1 = (results) => {
			...
			if (!results || typeof results !== "object" || !results.email ) {
				let newCallback = (createResult) => {
					if (createResult && createResult.insertId) {
						me.findById(createResult.insertId, originalCallback);
					}
				}
				item.save(newCallback);
		...
		console.log("running findOne with callback1");
		...
		pool.getConnection(function(err, connection) {
			...
			connection.query(queryOptions, function(err, results) {
				...
				if (err) {
					...
					return callback ("No record with with specified params  found in '" + me.relation + "' relation.");
				}
				if (typeof callback === "function") {
